...
一个二进制，一页浏览器界面。客户端是 Rust 编译成的 WebAssembly，嵌在这个二进制里；没有第二个客户端，构建链里没有 npm 也没有 node——这一条由一道门看着，不靠自觉。
...
3. **city**——盖一栋楼。
4. 底部的 control surface——地址、要产出什么、什么算完成。**它不问预算**：一件活跑之前没人说得出它值多少钱，订阅更是没有单价；花了多少事后从记录里报出来。
...
**能做**，每一条背后都有端到端断言或真实测量：注册 provider 并选模型；盖楼、派活；模型真实调用 tool 并把文件写进那栋楼；给一栋楼配外部 MCP server；多个 Agent 同时开工，各有自己的 git worktree，改动要别人验过才能并回楼里（这是编译错误，不是一条规矩）；十个页面（城市、直播、审批、回收站、归档、成本、账本、楼、房间信箱、设置）；停城与放行；离线验链；导出一座城并在另一台机器上恢复。

**建好了但没见过真东西**：hosted MCP server、真的订阅登录、真实 HTTP 上的 ACP 入站请求。三条链都只对着自己写的 server 验过——那证明的是链，不是对面。Anthropic dialect 有一次真实调用挂住没返回，原因未定；OpenAI dialect 干净。
