...
- **guard 同样区分「放松」与「收紧」，判据是 diff 的形状而不是目录（V3.35）**。`length` 门自己命令：一个回到预算之内的文件**必须**从 `[file_length.predating]` 划掉，否则它报「不再是例外」。于是拆分与划掉天然同住一枚提交，而旧口径会为每一次拆分收一次裁定。**划掉一行豁免放松不了任何东西**：那个文件从「只受自己那颗钉子约束」变成「与其他所有文件同受预算约束」。因此 `xtask/budgets.toml` 在**整份 diff 只删登记行**时不算门面，其余一律照旧。**只认删除**：新增一行是新的豁免；把一条超长签名的豁免搬到它移去的新地址也是新增——一条签名要么被修好要么原地不动，不得带着免死金牌搬家。
