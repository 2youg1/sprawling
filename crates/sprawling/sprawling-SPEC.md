...
| 行 | 长度 | 相位（§5 步） | 归位值 |
|---|---|---|---|
| 3631 | 46 | 准入＋简报＋job locator（步 3–4） | —（留在原地） |
| 3677 | 62 | 规则／配置／选型／身份／租约 | `Site` |
| 3739 | 75 | 五张 desk | `Desks` |
| 3814 | **214** | catalog＋十三件工具＋bench＋阅览室（步 6） | `Workbench`（需再分二：工具构造 / 阅览室） |
| 4028 | 85 | prefix＋RunPlan＋handoff（步 5） | 返 `RunPlan` |
| 4113 | 22 | 驱动调用（步 7–11） | ✓ 已完成 |
| 4135 | 114 | 结桌：signals／goals／扫尾／计划／书架（步 12） | — |
| 4249 | 106 | pr 五臂 | — |
| 4355 | 43 | 租约归还／taint／待批 | — |
| 4398 | 45 | 冻结＋派下去的活（步 13） | — |

注意：**工具那一块 214 行，自己就超过 200**，所以它不能搬成一个方法就算完——需拆为「造工具并登记」与「阅览室与缺书」两步。这也是尺寸门选 200 而不是更宽数字的一个副作用：它不允许把一堆东西搬到另一处冒充分解。
