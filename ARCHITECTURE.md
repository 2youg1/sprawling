...
| V5 formal | termination, absence of overflow, monotonicity | 4 of 11 kani harnesses, Linux CI — the four whose input domain is unbounded |
...
**Four gaps, named rather than hidden.** CI has no browser driver, so V9 is a command a developer runs rather than a gate. The isometric city compares display lists rather than bitmaps: the preconditions for bitmap comparison are paid for — placement is a pure function of the id, painter order is total, projection and its inverse are exact — but there is no rasteriser. And V6 stops below `bin::assembly` (§3): a seeded scenario reproduces a run, not a dispatch, because `RunWorker` builds its model adapter instead of receiving one. What holds the dispatch policy is that module's own tests, plus the integration tests the lib target makes possible. And seven of the eleven kani harnesses are written but not proved. Every run of them from the first to 2026-08-29 hit a ceiling — six hours, then forty-five minutes with a bounded unwind — on the same harness, because CBMC cannot bound the loops inside the `BTreeSet` a taint set is made of. Those seven are also the seven whose inputs are concrete, so each states something the `#[test]` beside it already states, twice over a wider domain; CI proves the four that carry an unbounded domain, and closing the gap means rewriting a harness to be symbolic without a heap collection rather than waiting longer (P4.05).
