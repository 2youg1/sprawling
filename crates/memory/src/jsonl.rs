...
//!
//! The Vfs seam is crate-internal on purpose: std fs
//! and FaultFs are its two adapters, and it never enters a public
//! signature — `JsonlLedger` hides it behind `Box<dyn Vfs>`.
...
/// Crate-internal filesystem seam. Two adapters: [`RealFs`] (std) and
/// `FaultFs` (fault injection, S1.08). Never public: the seam stays inner.
pub(crate) trait Vfs {
